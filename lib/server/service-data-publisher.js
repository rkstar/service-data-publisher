...
class __ServiceDataPublisher__ {
  ...
  updateServicesData(user){
    ...
    Meteor.users.update({_id: user._id},{$set:{
      services_data_lastUpdate: new Date(),
  ...
  sanitizeServicesData(services){
    ...
    _.keys(this.services).map((service)=>{
      if( services[service] ){
        ...
        if( _.isArray(services[service]) ){
          let sanitized_data_array = []
          services[service].map((account)=>{
            sanitized_data_array.push(translator(account))
          })
          sanitized_data[service] = sanitized_data_array
        } else {
...
export default new __ServiceDataPublisher__()